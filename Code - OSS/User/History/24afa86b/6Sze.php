...
if (isset($_POST['upload'])) {
    ...
    $query = "INSERT INTO upload (name, size, type, path, content) " .
             "VALUES ('$fileName', '$fileSize', '$fileType', '$filePath', 'images')";
          ...
          <div class="flex flex-col gap-10">
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    <p class="text-sm">'. $row["type"].'Matematika</p>
                  </td>';
                    echo'<td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                        <p class="text-black dark:text-white">'.$row["size"].'</p>
                      </td>';
                    echo "<td>" . $row["type"] . "</td>";
